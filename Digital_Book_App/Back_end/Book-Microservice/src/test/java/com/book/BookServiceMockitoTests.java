...
public class BookServiceMockitoTests {
	...
	public void test_createBook() {
		...
		assertEquals(bookService.createBook(bookinfodto, 1),book.getBookTitle() + " Created!");
	}
	...
	public void test_updateBookContent() {
		...
		assertEquals(null, bookService.updateBookContent(book, 1));
	...
	when(bookRepository.save(existingBook)).thenReturn(existingBook);
	assertEquals(book, bookService.updateBookStatus(book, 1));
	}
	...
	public void test_getAuthorNameByBookId() {
		...
		assertEquals(bookService.getAuthorNameByBookId(book.getBookId()),"Rishabh");
